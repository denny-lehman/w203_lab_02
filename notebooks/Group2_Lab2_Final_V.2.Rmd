...
model1 <- lm(log(area+1) ~ wind + rain_binary + FFMC + log(BUI), data = Prod)
coeftest(model1, vcov=vcovHAC)

# a model that uses all available variables
model2 <- lm(log(area+1) ~ wind + rain_binary + FFMC + log(BUI) + RH + temp, data = Prod)
coeftest(model2, vcov=vcovHAC)

# a model that uses only the direct atmospheric metrics
model3 <- lm(log(area+1) ~ wind + rain_binary + RH + temp, data = Prod)
